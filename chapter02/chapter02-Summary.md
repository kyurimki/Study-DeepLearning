...
### 2.2.3 가중치와 편향 구하기
- 가중치와 편향의 차이
    - 가중치 w1, w2: 각 입력 신호가 결과에 주는 영향력(중요도) 조절하는 매개변수
    - 편향 b: 뉴런이 얼마나 쉽게 활성화(결과가 1)하는가를 조정하는 매개변수
    - ex. if) b = -0.1, ((입력 신호) * (가중치)의 합)) > 0.1일 때 뉴런 활성화
    - ex. if) b = -20.0, ((입력 신호) * (가중치)의 합)) < 20.0이면 뉴런 활성화 X
```
**gatesWithBias.py**
import numpy as np
